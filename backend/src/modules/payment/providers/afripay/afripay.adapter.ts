...
export class AfriPayAdapter implements IPaymentProvider {
  public readonly name = 'afripay';
   ...
   */
  async initiatePayment(request: PaymentInitRequest, clientToken: string): Promise<PaymentInitResponse> {
    try {
   ...
   */
  async verifyTransaction(clientToken: string, transactionRef?: string): Promise<TransactionVerificationResult> {
    // In production, can query AfriPay verification endpoint if available.
    return {
      transactionId: clientToken,
      status: 'PENDING',
